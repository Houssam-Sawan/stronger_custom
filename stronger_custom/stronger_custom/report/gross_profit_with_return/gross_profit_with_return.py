...
def execute(filters=None):
	...
	#filters.currency = frappe.defaults.get_user_default("Company") #frappe.get_cached_value("Company", filters.company, "default_currency")
	filters["company"] ="Stronger Iraq"
...
class GrossProfitGenerator:
	...
	def get_buying_amount_from_product_bundle(self, row, product_bundle):
		...
		for packed_item in product_bundle:
			if packed_item.get("parent_detail_docname") == row.item_row:
				...
				packed_item_row.qty = flt(packed_item.qty) * -1
				packed_item_row.serial_no = packed_item.get("serial_no")
				packed_item_row.batch_no = packed_item.get("batch_no")
	...
	def get_average_buying_rate(self, row, item_code):
		...
		if key not in self.average_buying_rate:
			...
			if row.serial_no:
				args.update({"serial_no": row.serial_no})
				
			if row.batch_no:
				args.update({"batch_no": row.batch_no})
	...
	def load_invoice_items(self):
		...
		self.si_list = frappe.db.sql(
			...
			select
				...
				`tabSales Invoice Item`.cost_center, `tabSales Invoice Item`.serial_no, `tabSales Invoice Item`.batch_no
				{sales_person_cols}
				{payment_term_cols}
			from
		...
		if item_code and warehouse:
			if (item_code, warehouse) not in self.sle:
				...
				res = (
					...
					.orderby(sle.warehouse, sle.posting_date, sle.creation, order=Order.desc)
					.orderby(sle.warehouse, sle.posting_time, sle.creation, order=Order.desc)
					.run(as_dict=True)
				)
